        ...
        <form action="{{ route('bagi-hasil-bulanan.update', $bulanan) }}" method="POST">
            ...
            <div class="row g-3">
                <div class="col-md-4 mb-3">
                    ...
                    <select name="id_desa" id="id_desa" class="form-select text-kecil @error('id_desa') is-invalid @enderror" required>
                        <option value="">Pilih Desa</option>
                        @foreach ($desa as $d)
                            <option value="{{ $d->id_desa }}" {{ old('id_desa', $bulanan->id_desa) == $d->id_desa ? 'selected' : '' }}>
                                {{ $d->desa }}
                            </option>
                        @endforeach
                    </select>
                ...
                <div class="col-md-4 mb-3">
                    <label for="id_tahun_tanam" class="form-label">Tahun Tanam <span class="text-danger">*</span></label>
                    <select name="id_tahun_tanam" id="id_tahun_tanam" class="form-select text-kecil @error('id_tahun_tanam') is-invalid @enderror" required>
                        <option value="">Pilih Tahun Tanam</option>
                        @foreach ($tahunTanam as $t)
                            <option value="{{ $t->id_tahun_tanam }}" {{ old('id_tahun_tanam', $bulanan->id_tahun_tanam) == $t->id_tahun_tanam ? 'selected' : '' }}>
                                {{ $t->tahun }}
                            </option>
                        @endforeach
                    </select>
                ...
                <div class="col-md-4 mb-3">
                    ...
                    <input type="text" id="total_luas" class="form-control text-kecil" value="{{ $total_luas }} Ha" readonly>
                </div>
            </div>
            ...
            <div class="row g-3">
                <div class="col-md-4 mb-3">
                    ...
                    <select name="bulan" id="bulan" class="form-select text-kecil @error('bulan') is-invalid @enderror" required>
                        <option value="">Pilih Bulan</option>
                        @php
                            $namaBulan = ['Januari','Februari','Maret','April','Mei','Juni','Juli','Agustus','September','Oktober','November','Desember'];
                        @endphp
                        @foreach ($namaBulan as $index => $bulan)
                            <option value="{{ $index + 1 }}" {{ old('bulan', $bulanan->bulan) == $index + 1 ? 'selected' : '' }}>
                                {{ $bulan }}
                            </option>
                        @endforeach
                    </select>
                ...
                <div class="col-md-4 mb-3">
                    ...
                    <input type="number" name="tahun" class="form-control text-kecil @error('tahun') is-invalid @enderror" value="{{ old('tahun', $bulanan->tahun) }}" required>
                    @error('tahun')
                        <div class="invalid-feedback">{{ $message }}</div>
                    @enderror
                </div>
                ...
                <div class="col-md-4 mb-3">
                    ...
                    <input type="date" name="tanggal_bagi" class="form-control text-kecil @error('tanggal_bagi') is-invalid @enderror" value="{{ old('tanggal_bagi', $bulanan->tanggal_bagi) }}" required>
                    @error('tanggal_bagi')
                        <div class="invalid-feedback">{{ $message }}</div>
                    @enderror
                </div>
            ...
            <div class="mb-3">
                <label for="total_bagian" class="form-label">Total Bagian (20%) <span class="text-danger">*</span></label>
                <input type="number" name="total_bagian" class="form-control text-kecil @error('total_bagian') is-invalid @enderror" value="{{ old('total_bagian', $bulanan->total_bagian) }}" required>
                @error('total_bagian')
                    <div class="invalid-feedback">{{ $message }}</div>
                @enderror
            </div>
